...
import stormDamageImage from "./assets/storm-damage.png";
...
const MPESA_ACCOUNT = "105225";
const BANK_ACCOUNT = "1650280005225";
const BANK_NAME = "Equity Bank Kenya Limited";
const BANK_ACCOUNT_NAME =
  "SEVENTH DAY ADVENTIST CHURCH EAST AFRICAN UNION CLINIC SIX KAKUMA";
const EAZZY_PAY = "Equity Bank Kenya Limited";
...
const BUDGET_GROUPS = [
  ...
  },
  {
    ...
    color: "#8BA8C0",
    items: [{ label: "Contingency Buffer", pct: 3 }],
  },
];
...
const IMPACT_TIERS = [
  {
    ...
    icon: "🪨",
    description: "Covers one bag of cement or a meter of reinforcement steel",
  },
  {
    id: "t150",
    usd: 150,
    ...
    icon: "🏗️",
    description: "Funds a steel column section or a full roofing sheet panel",
  },
  {
    id: "t500",
    usd: 500,
    label: "Roofing Sponsor",
    icon: "🏛️",
    description:
      "Completes a roofing section and weather-sealing for 100 seats",
  },
  {
    id: "t1200",
    usd: 1200,
    ...
    icon: "✝️",
    description: "Full combined roofing + safe seating base for a worship row",
  },
];
...
const PAYMENT_METHODS = [
  {
    id: "mpesa",
    label: "M-Pesa (Equity EazzyPay)",
    icon: "📱",
    desc: EAZZY_PAY + " · Paybill " + PAYBILL,
  },
...
const _churchProject = projectCatalog.find(
  (p) => p.slug === "church-construction",
);
const CHURCH_BEFORE_IMG = stormDamageImage;
...
const SANCTUARY_SPECS = {
  ...
  area: 30 * 15, // 450 m²
  materials: [
    "Heavy-duty metal poles",
    "Structural steel bars",
    "Corrugated iron sheets (roof + walls)",
  ],
};
...
const CONSTRUCTION_MILESTONES = [
  {
    label: "Site Preparation & Foundation Leveling",
    pct: 60,
    status: "In Progress",
    color: "#0F2942",
  },
  {
    label: "Metal Pole Erection (Main Uprights)",
    pct: 35,
    status: "In Progress",
    color: "#0F2942",
  },
  {
    label: "Steel Bars, Purlins & Bracing",
    pct: 20,
    status: "In Progress",
    color: "#4E7C9B",
  },
  {
    label: "Iron Sheet Roofing Installation",
    pct: 0,
    status: "Pending Funding",
    color: "#D4AF37",
  },
  {
    label: "Iron Sheet Wall Cladding (All Sides)",
    pct: 0,
    status: "Pending Funding",
    color: "#8BA8C0",
  },
  {
    label: "Interior Fitting, Floor & Electrical",
    pct: 0,
    status: "Pending Funding",
    color: "#8BA8C0",
  },
...
function detectLocalCurrency() {
  ...
  const region =
    (navigator.language || "en-KE").split("-")[1]?.toUpperCase() || "KE";
  const map = {
    KE: "KES",
    UG: "UGX",
    TZ: "TZS",
...
function FormCard({ title, children, className, step, accent }) {
  return (
    ...
    >
      <div
        className={`flex items-center gap-3 px-6 pt-5 pb-3 border-b border-slate-100 ${accent ? "" : ""}`}
      >
        {step != null && (
          <span className="w-6 h-6 rounded-full bg-[#0F2942] text-white text-[10px] font-black flex items-center justify-center shrink-0">
            {step}
          </span>
        )}
        <h3 className="text-xs font-black text-[#0F2942] uppercase tracking-widest">
          {title}
        </h3>
      </div>
...
function PaymentRow({ label, value, onCopy, copied }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className="font-mono font-bold text-[#0F2942] text-sm truncate">
        {value}
      </span>
      <button
        type="button"
      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition ${
                    ...
                    : current
                      ? "bg-[#0F2942] text-white ring-2 ring-[#0F2942]/30"
            ...
            {i < STEPS.length - 1 && (
              <div
                className={`flex-1 h-0.5 mx-1 mb-5 rounded transition ${done ? "bg-[#D4AF37]" : "bg-slate-200"}`}
              />
            )}
          </React.Fragment>
        );
...
export default function DonatePage() {
  ...
  const amountChosen =
    activeTier !== "" || (customInput !== "" && Number(customInput) >= 1);
  ...
  const allCurrencies = useMemo(
    () =>
      getSupportedCurrencyCodes().map((c) =>
        resolveCurrencyMeta(c, browserLocale),
      ),
    [browserLocale],
  );
  ...
  const convertUsd = useCallback(
    (usd, code) => {
      ...
      const rate =
        typeof liveRate === "number" && Number.isFinite(liveRate)
          ? liveRate
          : meta.rate;
      const raw = usd * rate;
      return meta.decimals === 0
        ? Math.round(raw)
        : Number(raw.toFixed(meta.decimals));
    },
  ...
  const qrData = useMemo(() => {
    ...
    if (method === "mpesa")
      return `MPESA|Paybill:${PAYBILL}|Account:${MPESA_ACCOUNT}|${base}`;
    if (method === "bank")
      return `BANK|${BANK_NAME}|Account:${BANK_ACCOUNT}|${base}`;
    return `SWIFT|${SWIFT_CODE}|Account:${BANK_ACCOUNT}|${base}`;
    ...
    () =>
      BUDGET_GROUPS.map((grp) => {
        const items = grp.items.map((item) => ({
          ...item,
          amount: formatMoney(
            convertUsd((CAMPAIGN_GOAL * item.pct) / 100, currency),
            currency,
          ),
        }));
        const groupPct = grp.items.reduce((s, item) => s + item.pct, 0);
        const subtotalUsd = grp.items.reduce(
          (s, item) => s + (CAMPAIGN_GOAL * item.pct) / 100,
          0,
        );
        return {
          ...grp,
          items,
          groupPct,
          subtotal: formatMoney(convertUsd(subtotalUsd, currency), currency),
        };
      }),
    [currency, convertUsd, formatMoney],
  );
  ...
  useEffect(() => {
    const isKnown =
      searchPrefill.project &&
      PROJECT_OPTIONS.some((p) => p.id === searchPrefill.project);
  ...
  useEffect(() => {
    const fetchRates = async () => {
      ...
      try {
        ...
        /* keep static fallback rates on network failure */
      }
    };
  ...
  const validateForm = () => {
    const errs = {};
    if (amountUSD < 1)
      errs.amount = `Minimum donation is ${formatMoney(convertUsd(1, currency), currency)} (${formatMoney(1, "USD")})`;
    if (!form.name.trim()) errs.name = "Full name is required";
    if (!form.email.trim()) errs.email = "Email is required";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
      errs.email = "Enter a valid email";
    if (method === "mpesa" && !form.phone.trim())
      errs.phone = "Phone number required for M-Pesa";
    return errs;
  };
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "success") {
          ...
          setSubmitError(
            "Payment cancelled or failed. Use manual details below.",
          );
        }
      } catch {}
      if (count >= 20) {
        ...
        setPollStatus((s) => {
          if (s !== "success") {
            setSubmitError("Payment timed out. Use manual details below.");
            return "failed";
          }
          return s;
        });
    ...
    <div className="min-h-screen bg-[#F7F4EF] font-sans">
      ...
      <section className="relative overflow-hidden bg-[#0F2942] text-white">
            ...
            <div className="flex h-full">
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-b from-[#0F2942]/30 to-[#0F2942]/85 flex flex-col justify-end p-4">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/50 mb-0.5">
                    Today
                  </span>
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-b from-[#0F2942]/10 to-[#0F2942]/75 flex flex-col justify-end p-4">
                  <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-[#D4AF37]/80 mb-0.5">
                    Our Vision
                  </span>
                  <span className="text-xs font-bold text-[#D4AF37] leading-snug">
                    30m × 15m × 5m — iron sheet &amp; metal frame
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-4 py-10">
          ...
          >
            A <strong className="text-[#D4AF37]">30 m × 15 m</strong>{" "}
            metal-frame sanctuary —{" "}
            <strong className="text-[#D4AF37]">5 m</strong> walls to wall plate,{" "}
            <strong className="text-[#D4AF37]">4 m</strong> roof pitch —
            heavy-duty poles, structural bars, and iron sheets allover — rising
            for 868 members in Kakuma, Kenya.
          </motion.p>
          ...
          >
            <div className="flex justify-between text-sm font-semibold text-white/70 mb-2">
              <span>
                Raised:{" "}
                <span className="text-[#D4AF37]">{campaignRaisedDisplay}</span>
              </span>
              <span>
                Goal: <span className="text-white">{campaignGoalDisplay}</span>
              </span>
            </div>
            ...
            <p className="mt-2 text-xs text-white/55 text-center">
              {progressPct}% of Phase 1 goal reached
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-[#F7F4EF] py-12 px-4 border-b border-slate-200">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-[#0F2942]">
              Built to Endure: Our Permanent Sanctuary
            </h2>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            >
              {CHURCH_METAL_IMG ? (
                <img
                  src={CHURCH_METAL_IMG}
                  alt="High-duty metal poles and bars prepared for sanctuary construction"
                  className="w-full h-64 object-cover"
                />
              ) : (
                <div className="w-full h-64 bg-[#0F2942]/10 flex items-center justify-center text-[#0F2942]/30 text-sm">
                  Construction photo
                </div>
              )}
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#0F2942]/90 to-transparent px-5 py-4">
                <div className="flex items-end gap-4 flex-wrap">
                  ...
                  <div className="text-center">
                    <div className="text-base font-black text-[#D4AF37]/70">
                      +{SANCTUARY_SPECS.ridgeHeight}m
                    </div>
            ...
            >
              <div className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm">
                <p className="text-xs font-bold uppercase tracking-widest text-[#0F2942]/40 mb-3">
                  Construction Method
                </p>
                  ...
                  {SANCTUARY_SPECS.materials.map((m) => (
                    <li key={m} className="flex items-start gap-2.5">
                      <span className="mt-0.5 h-4 w-4 rounded-full bg-[#D4AF37] flex items-center justify-center shrink-0">
                        <svg
                          className="w-2.5 h-2.5 text-[#0F2942]"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="3"
                          viewBox="0 0 24 24"
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div
                    key={s.label}
                    className="rounded-xl bg-white border border-slate-200 px-4 py-3 shadow-sm"
                  >
                    <p className="text-[10px] uppercase tracking-widest text-[#0F2942]/40 mb-0.5">
                      {s.label}
                    </p>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-[#0F2942]/50 text-sm mt-2 max-w-lg mx-auto">
              Live construction stages for the 30 m × 15 m sanctuary (5 m wall
              plate + 4 m roof ridge) in Kakuma.
            </p>
          </motion.div>
            ...
            {CONSTRUCTION_MILESTONES.map((m, i) => (
              ...
              >
                <div className="flex items-center justify-between mb-2 gap-3">
                  <span className="text-sm font-bold text-[#0F2942] leading-snug">
                    {m.label}
                  </span>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span
                      ...
                      style={{ color: m.pct > 0 ? m.color : "#CBD5E1" }}
                    >
                      {m.pct}%
                    </span>
                  </div>
          ...
          >
            Wall cladding and interior fitting are pending funding. Your
            donation directly unlocks the next phase.
          </motion.p>
        </div>
      </section>
          ...
          {submitted ? (
            ...
            >
              <div className="bg-white rounded-2xl shadow-lg p-8 border border-[#D4AF37]/30">
                ...
                {pollStatus === "success" ? (
                  <div className="text-center">
                    ...
                    <h2 className="text-2xl font-black text-[#0F2942] mb-2">
                      Payment Confirmed!
                    </h2>
                    <p className="text-[#0F2942]/70 text-sm mb-4">
                      Thank you{form.anonymous ? "" : `, ${form.name}`}. Your
                      M-Pesa payment was received.
                    </p>
                    {mpesaReceipt && (
                ...
                pollStatus === "pending" ? (
                  ...
                  <div className="text-center py-2">
                    ...
                    <h2 className="text-xl font-black text-[#0F2942] mb-2">
                      Payment Prompt Sent!
                    </h2>
                    <p className="text-[#0F2942]/70 text-sm mb-4">
                      An <strong>Equity Bank EazzyPay</strong> payment prompt
                      has been sent to{" "}
                      ...
                      via M-Pesa.
                      <br />
                      Open your M-Pesa menu and <strong>
                        enter your PIN
                      </strong>{" "}
                      to authorise the payment.
                    </p>
                    <div className="inline-block bg-[#F7F4EF] rounded-xl px-6 py-3 mb-3 text-left">
                      <div className="text-[10px] uppercase tracking-widest text-[#0F2942]/40 mb-1">
                        Charging to {EAZZY_PAY}
                      </div>
                      <div className="text-2xl font-black text-[#0F2942]">
                        {formatMoney(amountInCurrency, currency)}
                      </div>
                      {currency !== "USD" && amountUSD > 0 && (
                        <div className="text-xs text-[#0F2942]/40">
                          ≈ {formatMoney(amountUSD, "USD")}
                        </div>
                      )}
                      <div className="mt-2 text-[10px] font-mono text-[#0F2942]/40">
                        Paybill {PAYBILL} · Acct {MPESA_ACCOUNT}
                      </div>
                    </div>
                    <p className="text-xs text-[#0F2942]/35 mb-5">
                  ...
                  <div className="py-2">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <h2 className="text-base font-black text-[#0F2942]">
                          Automatic Payment Not Confirmed
                        </h2>
                    ...
                    <div className="rounded-xl border-2 border-[#D4AF37]/60 bg-[#D4AF37]/5 p-4 mb-3">
                      <p className="text-xs font-black uppercase tracking-widest text-[#0F2942]/50 mb-2">
                        Option A — Try Again Automatically
                      </p>
                      <p className="text-sm text-[#0F2942]/70 mb-3">
                        We'll resend a prompt to{" "}
                        <span className="font-bold text-[#0F2942]">
                          {form.phone}
                        </span>
                        . Just enter your PIN when it arrives.
                      </p>
                      <button
                        type="button"
                        onClick={resendStkPush}
                    ...
                    <div className="flex items-center gap-3 my-3">
                      ...
                      <span className="text-xs font-bold text-[#0F2942]/35 uppercase tracking-widest">
                        or
                      </span>
                      <div className="flex-1 h-px bg-slate-200" />
                    </div>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-[#F7F4EF] p-4">
                      <p className="text-xs font-black uppercase tracking-widest text-[#0F2942]/50 mb-3">
                        Option B — Pay Manually via M-Pesa
                      </p>
                      <div className="space-y-2.5 mb-3">
                        <PaymentRow
                          label="Paybill"
                          value={PAYBILL}
                          onCopy={() => copyToClipboard(PAYBILL, "Paybill")}
                        ...
                        <PaymentRow
                          ...
                          value={MPESA_ACCOUNT}
                          onCopy={() =>
                            copyToClipboard(MPESA_ACCOUNT, "Account")
                          }
                        ...
                        <div className="text-xs font-mono text-[#0F2942]/40">
                          Ref: {paymentRef}
                        </div>
                      </div>
                      <p className="text-[11px] text-[#0F2942]/50 leading-relaxed">
                        Go to{" "}
                        <strong>M-Pesa → Lipa na M-Pesa → Pay Bill</strong>,
                        enter the numbers above, then use{" "}
                        <span className="font-mono">{paymentRef}</span> as your
                        account reference.
                      </p>
                      {isMobileDevice() && (
                        <a
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <div>
                        <h2 className="text-lg font-black text-[#0F2942]">
                          Thank you
                          {form.name && !form.anonymous ? `, ${form.name}` : ""}
                          !
                        </h2>
                        <p className="text-sm text-[#0F2942]/55">
                          Complete your gift using the details below.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="rounded-xl bg-[#F7F4EF] border border-[#D4AF37]/40 p-5 space-y-3 mb-5">
                      ...
                      {method === "bank" && (
                        <>
                          <PaymentRow
                            label="Bank"
                            value={BANK_NAME}
                            onCopy={() => copyToClipboard(BANK_NAME, "Bank")}
                            copied={copied === "Bank"}
                          />
                          <PaymentRow
                            label="Account"
                            value={BANK_ACCOUNT}
                            onCopy={() =>
                              copyToClipboard(BANK_ACCOUNT, "Account")
                            }
                            copied={copied === "Account"}
                          />
                        </>
                      ...
                      {method === "wire" && (
                        <>
                          <PaymentRow
                            label="SWIFT"
                            value={SWIFT_CODE}
                            onCopy={() => copyToClipboard(SWIFT_CODE, "SWIFT")}
                            copied={copied === "SWIFT"}
                          />
                          <PaymentRow
                            label="Account"
                      ...
                      <div className="pt-1 text-xs font-mono text-[#0F2942]/40">
                        Ref: {paymentRef}
                      </div>
                    </div>

                    <p className="text-xs text-[#0F2942]/55 italic leading-relaxed mb-3">
                      Once your transfer is received, a confirmation receipt
                      will be sent to <strong>{form.email}</strong>. You may
                      also email us at{" "}
                      <a
                        href={`mailto:${CHURCH_EMAIL}`}
                        className="text-[#D4AF37] font-semibold not-italic hover:underline"
                      >
                        {CHURCH_EMAIL}
                      </a>{" "}
                      with your transfer reference and we will acknowledge your
                      gift within 24 hours. These funds are freewill offerings
                      above tithe, managed through church treasury controls.
                    </p>
                ...
                {(pollStatus === "success" || pollStatus === "") && (
                  <div className="mt-5 flex gap-3">
                    <button
                      className="flex-1 py-2.5 rounded-xl border border-[#0F2942]/20 text-sm text-[#0F2942] font-semibold hover:border-[#D4AF37] transition"
                      onClick={resetForm}
                    >
                      Give Again
                    </button>
                    <Link
                      to="/"
                      className="flex-1 py-2.5 rounded-xl bg-[#0F2942] text-white text-sm font-semibold text-center hover:bg-[#0F2942]/90 transition"
                    >
                      Back to Home
                    </Link>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8 border border-[#D4AF37]/30 flex flex-col items-center justify-center gap-4">
                <p className="text-xs font-bold uppercase tracking-widest text-[#0F2942]/40">
                  Scan to Pay
                </p>
                ...
                <p className="text-[10px] font-mono text-[#0F2942]/30">
                  {paymentRef}
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="relative bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  >
                    <span className="text-[10px] font-black uppercase tracking-widest text-[#0F2942]/40 shrink-0">
                      Display in
                    </span>
                    <div className="flex items-center gap-2 ml-auto min-w-0">
                      ...
                      <span className="font-black text-[#0F2942] text-sm shrink-0">
                        {activeCurrencyMeta.code}
                      </span>
                      <span className="text-xs text-[#0F2942]/45 truncate hidden sm:block">
                        {activeCurrencyMeta.name}
                      </span>
                      {currency !== "USD" && (
                      ...
                      <span className="flex items-center gap-1 shrink-0 ml-1">
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${ratesAge ? "bg-green-500" : "bg-yellow-400"}`}
                        />
                        <span className="text-[10px] text-[#0F2942]/35">
                          {ratesAge ? "live" : "est."}
                        </span>
                      </span>
                      <span className="text-xs text-[#D4AF37] font-bold ml-2 shrink-0">
                        {showCurrency ? "▲" : "▼"}
                      </span>
                    </div>
                  </button>
                        ...
                        <div className="px-5 pb-4 pt-3">
                                      ...
                                      <button
                                        ...
                                        className={`rounded-lg border text-left px-2 py-1.5 transition ${
                                          active
                                            ...
                                            : "bg-white text-[#0F2942] border-[#D4AF37]/35 hover:border-[#D4AF37]"
                                        }`}
                                      >
                                        <div className="flex items-center gap-1 text-xs font-bold">
                                          {meta.countryCode && (
                                            <img
                                              ...
                                              alt=""
                                              className="h-[10px] w-[14px] rounded-[2px] object-cover"
                                              loading="lazy"
                                            />
                                          )}
                                          {meta.code}
                          ...
                          <div className="max-h-52 overflow-y-auto rounded-xl border border-[#D4AF37]/25 divide-y divide-slate-100 bg-white">
                                ...
                                >
                                  {meta.countryCode ? (
                                    <img
                                      src={`https://flagcdn.com/24x18/${meta.countryCode}.png`}
                                      alt=""
                                      className="h-[11px] w-[15px] rounded-[2px] object-cover border border-black/10 shrink-0"
                                      loading="lazy"
                                    />
                                  ) : (
                                    <span className="inline-flex h-[11px] w-[15px] items-center justify-center text-[8px] font-bold bg-slate-200 rounded-[2px] shrink-0">
                                      {meta.code.slice(0, 1)}
                                    </span>
                                  )}
                                  ...
                                  <span
                                    className={`text-[10px] truncate ${active ? "text-white/65" : "text-[#0F2942]/55"}`}
                                  >
                                    {meta.name}
                                  </span>
                                </button>
                            ...
                            {filteredCurrencies.length === 0 && (
                              <div className="px-3 py-4 text-xs text-[#0F2942]/45 text-center">
                                No results for "{currencyQuery}"
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <FormCard title="Select an Impact Tier" step={1}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {IMPACT_TIERS.map((tier) => {
                      const active =
                        activeTier === tier.id && customInput === "";
                      return (
                        <button
                          key={tier.id}
                          type="button"
                          ...
                          className={`relative text-left rounded-xl border-2 px-4 py-4 transition ${
                            active
                              ...
                              : "border-slate-200 bg-white hover:border-[#D4AF37]/60 hover:shadow-sm"
                          }`}
                        >
                          {active && (
                          ...
                          <div className="font-black text-[#0F2942] text-sm">
                            {tier.label}
                          </div>
                          <div
                            className={`font-black text-base mt-0.5 ${active ? "text-[#8B6914]" : "text-[#D4AF37]"}`}
                          >
                            {formatMoney(
                              convertUsd(tier.usd, currency),
                              currency,
                            )}
                          </div>
                          <p className="mt-1.5 text-[11px] text-[#0F2942]/50 leading-relaxed">
                            {tier.description}
                          </p>
                        </button>
                      );
                ...
                <FormCard title="Or Enter a Custom Amount" step={2}>
                  ...
                  {customInput !== "" && Number(customInput) < 1 && (
                    <p className="mt-1.5 text-xs text-red-600">
                      ...
                      {formatMoney(convertUsd(1, currency), currency)}
                    </p>
                  )}
                  {amountInCurrency > 0 && (
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-sm font-bold text-[#0F2942]">
                        {formatMoney(amountInCurrency, currency)}
                        {currency !== "USD" && amountUSD > 0 && (
                          <span className="ml-2 text-xs font-normal text-[#0F2942]/40">
                            ≈ {formatMoney(amountUSD, "USD")}
                          </span>
                        )}
                      </span>
                        ...
                        <button
                          ...
                          className={`text-left rounded-xl border-2 px-4 py-3 transition ${
                            active
                              ...
                              : "border-slate-200 bg-white text-[#0F2942] hover:border-[#D4AF37]"
                          }`}
                        >
                          ...
                          {p.shortSummary && (
                            <p
                              className={`mt-1 text-[11px] leading-relaxed line-clamp-2 ${active ? "text-white/70" : "text-[#0F2942]/50"}`}
                            >
                              {p.shortSummary}
                            </p>
                          )}
                ...
                <FormCard title="Giving Frequency" step={4}>
                  <div className="inline-flex rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                    {[
                      { val: false, label: "One-time" },
                      { val: true, label: "Monthly" },
                    ].map((opt) => (
                      <button
                        key={String(opt.val)}
                        type="button"
                  ...
                  {recurring && (
                    <p className="mt-2 text-xs text-[#D4AF37] font-semibold">
                      Your gift will repeat monthly — cancel anytime by
                      contacting us.
                    </p>
                  )}
                </FormCard>
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      {method === "mpesa" && (
                        <p className="mt-1.5 text-xs text-[#0F2942]/50 flex items-start gap-1.5">
                          ...
                          <span>
                            An <strong>Equity Bank EazzyPay</strong> payment
                            prompt will be pushed to this number via M-Pesa. You
                            only enter your PIN to authorise — no account
                            numbers to copy.
                          </span>
                        </p>
                      )}
                    ...
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                      ...
                      <span className="text-sm text-[#0F2942]/70">
                        Give anonymously{" "}
                        <span className="text-[#0F2942]/40 text-xs">
              ...
              <div className="space-y-6 lg:sticky lg:top-6 self-start">
                ...
                <div className="bg-gradient-to-br from-[#0B1F34] to-[#0F2942] rounded-2xl p-6 text-white shadow-xl border border-[#D4AF37]/20">
                  <div className="text-xs uppercase tracking-widest text-[#D4AF37] font-bold mb-3">
                    Your Gift Summary
                  </div>
                  <div className="space-y-2 text-sm mb-4">
                    <div className="flex justify-between items-center">
                      <span className="text-white/55">Amount</span>
                      <span className="font-black text-lg text-[#D4AF37]">
                        {amountInCurrency > 0
                          ? formatMoney(amountInCurrency, currency)
                          : "—"}
                      </span>
                    ...
                    <div className="border-t border-white/10 pt-2 mt-1 space-y-1.5">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {PAYMENT_METHODS.find((pm) => pm.id === method)?.icon}{" "}
                          {
                            PAYMENT_METHODS.find((pm) => pm.id === method)
                              ?.label
                          }
                        </span>
                      </div>
                      {method === "mpesa" && (
                        <div className="mt-2 rounded-lg bg-white/[0.06] border border-white/10 px-3 py-2 text-xs space-y-1">
                          <div className="text-[10px] font-bold uppercase tracking-widest text-[#D4AF37]/60 mb-1">
                            {EAZZY_PAY}
                          </div>
                          <div className="flex justify-between font-mono">
                            <span className="text-white/40">Paybill</span>
                          ...
                          <p className="text-[10px] text-white/35 mt-1">
                            Funds deposited directly into Equity Bank
                          </p>
                        </div>
                      )}
                    </div>
                    {impactText && (
                      <p className="text-[11px] text-[#D4AF37]/80 font-semibold mt-1">
                        {impactText}
                      </p>
                    )}
                  </div>
                  ...
                  <p className="mt-3 text-[10px] text-white/30 text-center leading-relaxed">
                    🔒 Secure · Freewill offering above tithe · 100% reaches
                    ministry
                  </p>
                </div>
                    ...
                    {PAYMENT_METHODS.map((pm) => (
                      ...
                      >
                        ...
                        <div className="text-left">
                          ...
                          <div className="text-xs text-[#0F2942]/45">
                            {pm.desc}
                          </div>
                        </div>
                        <div
                  ...
                  {method === "mpesa" && (
                    <div className="mt-3 rounded-xl bg-[#EFF6FF] border border-[#0F2942]/12 px-4 py-3">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs font-black text-[#0F2942]/70">
                          {EAZZY_PAY}
                        </p>
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-[#0F2942]/55">
                            M-Pesa Paybill
                          </span>
                          <span className="font-mono font-black text-[#0F2942] text-sm">
                            {PAYBILL}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-mono font-bold text-[#0F2942] text-xs">
                            {MPESA_ACCOUNT}
                          </span>
                        </div>
                      </div>
                      <p className="mt-2 text-[10px] text-[#0F2942]/50 leading-relaxed">
                        📱 We send a payment prompt to your phone — you just
                        enter your M-Pesa PIN to authorise. No manual entry
                        needed. Your donation lands directly in{" "}
                ...
                {amountInCurrency > 0 && (
                  <div className="bg-white rounded-2xl border border-[#D4AF37]/30 p-5 flex flex-col items-center shadow-sm">
                    <p className="text-xs font-bold uppercase tracking-widest text-[#0F2942]/35 mb-3">
                      Scan to Pay
                    </p>
      ...
      {!submitted && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200 shadow-2xl px-4 py-3 flex items-center gap-3">
          <div className="flex-1 min-w-0">
            <div className="text-xs text-[#0F2942]/50 font-medium">
              Your gift
            </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className="text-white/60 text-sm max-w-xl mx-auto">
              Annual construction fund allocation — every line item tracked and
              audited.
            </p>
          </motion.div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 px-1">
            <div>
              ...
              {ratesAge && (
                <p className="text-[10px] text-white/30 mt-0.5">
                  🔄 Live rates · updated{" "}
                  {Math.round((Date.now() - ratesAge.getTime()) / 60000) < 1
                    ? "just now"
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-xs text-white/50 bg-white/[0.06] rounded-full px-3 py-1.5 font-mono flex items-center gap-1.5">
                <span
                  className={`inline-block w-1.5 h-1.5 rounded-full ${ratesAge ? "bg-green-400" : "bg-yellow-400"}`}
                />
                1 USD ≈{" "}
                {currency === "USD"
                  ? "1"
                ...
                {currency}
                {!ratesAge && (
                  <span className="text-yellow-400/70 ml-1">(est.)</span>
                )}
              </span>
              {/* Currency picker */}
              <div className="relative">
                <button
                  type="button"
                  onClick={() => {
                    setShowBudgetCurrency((s) => !s);
                    setBudgetCurrencyQuery("");
                  }}
                ...
                >
                  {activeCurrencyMeta.countryCode && (
                    <img
                      src={`https://flagcdn.com/24x18/${activeCurrencyMeta.countryCode}.png`}
                      alt=""
                      className="h-[12px] w-[17px] rounded-[2px] object-cover"
                      loading="lazy"
                    />
                  )}
                  <span>{currency}</span>
                  ...
                  <span className="text-[#D4AF37] text-xs ml-1">
                    {showBudgetCurrency ? "▲" : "▼"}
                  </span>
                </button>
                <AnimatePresence>
                            ...
                            >
                              {meta.countryCode ? (
                                <img
                                  src={`https://flagcdn.com/24x18/${meta.countryCode}.png`}
                                  alt=""
                                  className="h-[11px] w-[15px] rounded-[2px] object-cover border border-white/15 shrink-0"
                                  loading="lazy"
                                />
                              ) : (
                                <span className="inline-flex h-[11px] w-[15px] items-center justify-center text-[8px] font-bold bg-white/20 rounded-[2px] shrink-0">
                                  {meta.code.slice(0, 1)}
                                </span>
                              )}
                              <span className="text-xs font-bold w-8 shrink-0">
                              ...
                              <span
                                className={`text-[10px] truncate ${active ? "text-[#D4AF37]/75" : "text-white/50"}`}
                              >
                                {meta.name}
                              </span>
                            </button>
          ...
          >
            ...
            {budgetGroupsDisplay.map((grp, gi) => (
              <div key={grp.group}>
                ...
                {grp.items.map((item, ii) => (
                  ...
                  >
                    <span className="text-sm text-white/75 pl-3">
                      {item.label}
                    </span>
                    <span className="text-sm text-center w-20 text-[#D4AF37] font-bold">
                      {item.pct}%
                    </span>
                    <span className="text-sm text-right w-36 font-mono text-white/65">
                      {item.amount}
                    </span>
                  </motion.div>
                ))}
                ...
                <div className="grid grid-cols-[1fr_auto_auto] px-5 py-2.5 bg-white/[0.04] border-b border-white/10">
                  <span className="text-xs font-black text-white/45 pl-3 uppercase tracking-wider">
                    Subtotal
                  </span>
                  <span className="text-xs font-black text-center w-20 text-white/55">
                    {grp.groupPct}%
                  </span>
                  <span className="text-xs font-black text-right w-36 font-mono text-white/55">
                    {grp.subtotal}
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-[1fr_auto_auto] px-5 py-4 bg-[#D4AF37]/10 border-t-2 border-[#D4AF37]/30">
              ...
              <span className="font-black text-center w-20 text-[#D4AF37]">
                100%
              </span>
          ...
          <motion.div
            ...
            transition={{ duration: 0.6, delay: 0.3 }}
          >
            Clinic 6 SDA Church publishes annual construction audits via the
            Turkana County Field treasury. Currency conversions use indicative
            exchange rates and are shown for reference only.{" "}
            <strong className="text-[#D4AF37]">
              All donations are received and processed in USD.
            </strong>{" "}
            <strong className="text-[#D4AF37]">Tithe Boundary:</strong> These
            are freewill offerings above the systematic tithe — tithes must be
            returned through your home congregation's official Field/Conference
            treasury channel.
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-[#F7F4EF] py-12 px-4 border-t border-slate-200">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-[#0F2942]/35 font-bold mb-6">
            Under the Oversight of
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            {[
              "East Africa Division · SDA",
              "Turkana County Field",
              "Kakuma District",
              "SDA Clinic 6 Treasury",
            ].map((p) => (
              <span
                key={p}
                className="px-4 py-2 rounded-full border border-[#0F2942]/12 bg-white text-sm font-semibold text-[#0F2942]/45"
              >
                {p}
              </span>
            ))}
